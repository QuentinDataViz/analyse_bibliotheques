...
read_xlsx("analyse bibliotheques/data/2024 departements.xlsx", sheet = "Départements", skip = 7) %>%  
  clean_names() %>% 
  tibble() %>% 
...
pop_com <- 
  ...
  fromJSON("analyse bibliotheques/data/2024 communes.json") %>% 
  clean_names() %>% 
  tibble() %>% 
...
biblio_def <- 
  ...
  left_join(pop_dep %>% select(code_dep, ptot), join_by(code_departement == code_dep)) %>% 
